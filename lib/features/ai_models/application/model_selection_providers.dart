...
final activeModelSelectionProvider =
    FutureProvider<ActiveModelSelection>((ref) {
  return guardSensitiveFuture<ActiveModelSelection>(
    ...
    load: () async {
      ...
      final runtimeStates =
          await ref.watch(embeddingRuntimeStatesProvider.future);
  ...
  return guardSensitiveFuture<ModelRegistryEntry?>(
    ...
    load: () async {
      final selectedRuntime =
          await ref.watch(activeEmbeddingRuntimeSelectionProvider.future);
...
final semanticSearchReadinessProvider =
    ...
    load: () async {
      ...
      if (!runtimeState.ready) {
        return SemanticSearchReadiness(
          ready: false,
          reason:
              _runtimeBlockedReason(activeEmbeddingModel.name, runtimeState),
...
class SelectedEmbeddingRuntime {
  const SelectedEmbeddingRuntime(
      {required this.entry, required this.runtimeState});
...
String _runtimeBlockedReason(
    String modelName, EmbeddingEngineState runtimeState) {
